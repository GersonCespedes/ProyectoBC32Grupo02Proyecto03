...
public class BusinessRestController {
    private static final Logger log = LoggerFactory.getLogger(BusinessRestController.class);
    @Autowired
        ...
        return monoCustomer.flatMap(customer -> {
            return service.save(customer).map(p -> {
                ...
                return ResponseEntity
                        .created(URI.create("/api/customerbusiness/".concat(customer.getId())))
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(result);
            });
        }).onErrorResume(err -> {
    ...
    @PutMapping("/{id}")
    public Mono<ResponseEntity<CustomerBusiness>> edit(@RequestBody CustomerBusiness customer, @PathVariable String id) {
